...
export class Game {
  ...
  public moveTo(playerTile: Tile) {
    ...
    if (!this.decoy.active) this.decoy.tile = p.tile;
  }
  ...
  public endTurn() {
    ...
    /// Clear
    if (!this.over) {
      if (this.frozenTurns > 0) this.frozenTurns--;
      for (let bot of this.bots) bot.stunned = false;
  ...
  public recheck() {
    ...
    for (let tile of this.tiles) {
      if (!tile.busted) {
        ...
        if (bust) {
          for (let bot of bots) this.killBot(bot, true);
          tile.busted = true;
