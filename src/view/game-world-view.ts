...
export class GameWorldView extends Phaser.GameObjects.Container {
  ...
  public get viewW() {
    return this.TILESIZE * this.game.W;
  }
  public get viewH() {
    return this.TILESIZE * this.game.H;
  }

  public getTileX(v) {
    return this.TILESIZE * (v - this.game.W * 0.5 + 0.5);
  }
  public getTileY(v) {
    return this.TILESIZE * (v - this.game.H * 0.5 + 0.5);
  }
  public getActorX(v) {
    return this.getTileX(v);
  }
  public getActorY(v) {
    return this.getTileY(v) - 10;
  }
...
class TileSprite extends Phaser.GameObjects.Image {
  ...
  constructor(
    ...
    if (odd) {
      let h = Phaser.Math.FloatBetween(0.05, 0.07) + 0.5;
      let s = Phaser.Math.FloatBetween(0.48, 0.66);
      let l = Phaser.Math.FloatBetween(0.62, 0.72);
      this.setTint(Phaser.Display.Color.HSLToColor(h, s, l).color);
    } else {
      let h = Phaser.Math.FloatBetween(0.09, 0.11) + 0.4;
      let s = Phaser.Math.FloatBetween(0.44, 0.72);
      let l = Phaser.Math.FloatBetween(0.77, 0.82);
      this.setTint(Phaser.Display.Color.HSLToColor(h, s, l).color);
    }
  }
}
