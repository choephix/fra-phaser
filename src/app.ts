...
export class App {
  ...
  onAssetsLoaded() {
    ...
    App.ctrl = new AbstractTouchController((x, y) =>
      App.phaser_game.input.events.emit("move", x, y),
    );
    ...
    App.gameplay.events.on(GameEvent.GAMEOVER, (victory) =>
      App.phaser_game.scene.wake(
        victory ? "splash-continue" : "splash-gameover",
      ),
    );
...
export class BootScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.spritesheet("sheet_b", "fra.png", {
      frameWidth: 769,
      frameHeight: 500,
    });
  }
...
export class GameWorldScene extends Phaser.Scene {
  ...
  create_animations() {
    ...
    this.anims.create({
      ...
      repeat: -1,
      frameRate: 10,
      frames: this.anims.generateFrameNumbers("sheet_b", {
        start: 16,
        end: 19,
      }),
    });
